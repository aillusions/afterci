# Терапія та реабілітація дітей після кохлеарної імплантації

**Інформація, поради, методичні матеріали батькам дітей з кохлеарним імплантом** 

---


## Слухомовний розвиток

Вік | Статус
------------ | -------------
0-3 міс | Verbal play through cooing, gooing and laughing. Реагує на звук поворотом голови, здригається від раптового шуму, заспокоюється на знайомий голос, гулить, агукає, починає відрізняти одні звуки від інших
4-6 міс | Babbling begins. Іноді реагує на своє ім'я, часто визначає джерело звуку поворотом голови, слухає власний голос, уважніше слухає інших, лепече, гулить на одинці або поряд з іншими, попискує, вимовляє деякі приголосні
7-9 міс | Знаходить джерело звуку, співвідносить слова з їх значенням (understands that the word 'ball' means the object 'ball'), може виділити імена членів родини, мже припиняти діяльність почувши своє ім'я, визначає місцеположення предметів почувши їх назву, розуміє знчення "ні", "не можна"
10-12 міс | Syllable variation (e.g. badugatadudah). First words approximations (e.g. dada for daddy). Знаходить джерело звуку на відстані. Виділяє голос з фонових щвуків. Співвідносить більшу частину слів з їх значенням, може виконувати прості команди та прохання (іноді може дати іграшку на вербальне прохання, іноді виконати: 'поклади на місце'). Реагує на музику рухами. 'Розмовляє' з іграшками копіюючи інтонацію доросних. Чергує голосні та приголосні. Хитає головою в знак заперечної відповіді. Подає голос на поклик матері.  
13-15 міс | Виконує знайомі вказівки на одну дію. Розуміє прості запитання, наприклад: де (тато)? Розпізнає назви частин тіла. Використовує 7+ слів (10-15 words). Імітує слова, співає. Починає розуміти питання: що, де, коли, як.



[Концепції](misc/ideas.md)

[Загальний опис методик](methods/method.md)

[Календар розвитку](misc/calendar.md)

[Прродні жести](misc/sign_lang.md)

[Словник](misc/vocab.md)

[Завантаження](misc/downloads.md)

[Про імплантацію](misc/ci.md)

[Посилання](misc/refs.md)

[Контакти](misc/contacts.md)

[Технічні засоби](misc/tech.md)



